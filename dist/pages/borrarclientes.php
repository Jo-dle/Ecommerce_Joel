<?php
include_once "./controladores/clientes/borrarclientescontroladores.php";
?>
<!-- Content Header (Page header) -->
    <section class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <script>
              $(".alert").alert();
            </script>
          </div>
        </div>
      </div><!-- /.container-fluid -->
    </section>

    <!-- Main content -->
    <section class="content">
      <div class="container-fluid">
      
        <div class="row">
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                 <h3 class="card-title">Gestor  ecommerce</h3>
              </div>

              <!-- /.card-header -->
              <div class="card-body">
                <h1>Borrar Clientes</h1>  
              
                <form method="GET" action="index.php?modulo=clientes">
                <div class ="form-group">
                <input type="hidden" name="id" value="<?php echo htmlspecialchars($row['id']) ?>" required>
                  <label for ="nombre"> Nombre:</label>
                  <input type ="text" name="nombre" class ="form-control" value ="<?php print $row['nombre'] ?>" required>
                </div>
                <div class ="form-group">
                  <label for ="email"> Email:</label>
                  <input type ="text" name="email" class ="form-control" value ="<?php print $row['email'] ?>" required>
                </div>
              <div class="form group">
                <input type="hidden" name="modulo" value="borrarclientes">
                <button type="submit" name="borrar" VALUE="" class="btn btn-primary"> Borrar</button>
                <a class="btn btn-danger" href="index.php?modulo=clientes" role ="button">Cancelar</a>
              </div> 
            </form>      
              <!-- /.card-header -->
                </div>
              <!-- /.card-body -->
            </div>
            <!-- /.card -->
            <!-- /.card -->
          </div>
          <!-- /.col -->
        </div>
        <!-- /.row -->
      </div>
      <!-- /.container-fluid -->
    </section>
    <!-- /.content -->
  </div>
                 